    ...
    while True:
        ...
        if client_info == "Connection is from Client":
            ...
            if os.path.exists(file_path) and os.path.isfile(file_path):
                    ...
                    file_from_server2 = forward_request_to_server2(filename,file_path)
                    ...
                    # # client_socket.connect((SERV_2_HOST, SERV_2_PORT))
                    # # client_socket.bind((SERV_2_HOST, SERV_2_PORT))
                    # client_socket.send("File found on Server-1".encode())
                    # # # Send the filename to Server 2
                    # server_2.send(filename.encode())
                    # server_2.send(F_DIRECTORY.encode())
            ...
            # print(f"Oops..! File '{filename}' not found on Server-1. Forwarding file to Server 2.")
...
def forward_request_to_server2(filename,filepath):
